        ...
        def forward(self, mode='usual'):
            ...
            for agv_id in agvs_ids_list:
                if mode == 'usual':
                    ...
                    elif self.agvs_paths[agv_id][0] == self.agvs_paths[agv_id][-1]: #AGV到达了终点,等待
        ...
        def onload(self):
            ...
            while agv_id<len(self.agvs) and cargo_id<len(self.cargos):
                if self.agvs[agv_id].payload != -1 or self.agvs[agv_id].occupy != -1: #当前AVG在装载或是正在去装载的路上
                    if self.agvs_paths[agv_id] != [] or self.agvs[agv_id].payload != -1: #已经在运载了，就不给装货的重新拟定路线      
                        agv_id += 1
                        continue
                if self.shelfs[self.cargos[cargo_id].target].payload == cargo_id or self.cargos[cargo_id].load != -1:#货物已经在货架上了或是正在被AVG前往装载
                    if self.cargos[cargo_id].load != -1 and self.agvs_paths[self.cargos[cargo_id].load] != []:
                        cargo_id += 1
                        continue
